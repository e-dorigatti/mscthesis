...
\label{sec:profiles}
The wind and temperature profiles used to compute the flux-profile relationships are computed at 40 and 20 meters, because the turbulent surface fluxes present and surface friction velocity in the Cesar database are constant only within the surface layer. It is very hard to know the exact height of the surface layer, because it depends on the weather and no exact formulas are known [citation], but it is usually assumed to be 10\% as high as the boundary layer [citation]. Based on its typical heights [citation], the surface layer is often higher than 40 meters and lower than 80. \cite{cabauw_z0} indeed reports that a large number of observations at \SI{100}{\meter} are already outside of the surface layer. The wind speed is \SI{0}{\meter\per\second} at the height $z_0$, called \emph{characteristic roughness}. Its value depends on the properties of the surface, but no unambiguous value is known for the Cabauw observatory, especially since it depends on the direction of the wind \cite{cabauw_night, cabauw_z0}.

We use two methods to compute the gradient: difference quotients and a log-profile model introduced in \cite{windlogprofile}.

 The gradient is computed with finite differencing using measurements from the neighboring levels. The bottom-most measurements are at \SI{1.5}{\meter} for the temperature, and \SI{10}{meters} for the wind. Let $z_i$ be the height in meters of level $i$ and $a_i$ the corresponding measurement of quantity $a$, $\Delta^+a_i=a_{i+1}-a_i$ and $\Delta^-a_i=a_i-a_{i-1}$ and similarly for $z$. Then the gradient of $a$ at level $i$ is computed as the average of the top and bottom gradients, weighted by the distance between the levels:
...
Note that more advanced methods to obtain $\partial\overline{u}/\partial z$ can be used in stable conditions, where the wind profile is known to increase logarithmically with altitude; in this case, fitting $\overline{u}(z)=a+bz+cz^2+d\ln z$ to the profile yields an uncertainty in the estimate of the gradients $\partial\overline{u}/\partial z=b+2cz+d/z$ of around 30\% \cite{windlogprofile}.
