...
\begin{table}
\caption{Descriptive statistics of the best MSE achieved by each estimator among all feature sets with and without trend, and effect size comparing it with the MOST baseline. A positive effect size denotes improvement, and its 95\% right CI is in parenthesis. Note that we selected the best feature set comparing only the mean MSE, it is possible that another feature set with a slightly higher average but lower variance would generate a larger effect size.}
...
\begin{table}
\caption{Effect sizes and their 95\% right CI comparing the MSE scores of each feature set with and without trend. A positive effect size indicates that the trend reduces the error; in bold are the combinations whose CI does not contain 0.}
