...
\For{$i\gets 1\ldots I$}\Comment{Outer $I$-fold cross validation}
	\State{Generate $i$-th outer fold $(D^o_T,D^o_V)$ from the dataset $D$}
	\State{$best\_mse\gets\inf$}
	\State{$best\_params\gets\bot$}
	\For{$j\gets 1\ldots J$}\Comment{Find best hyper-parameters on $D^o_T$}
		\State{$params\gets$ a random hyper-parameter combination}
		\State{$params\_mse\gets 0$}
		\For{$k\gets 1\ldots K$}\Comment{Evaluate $params$ with $K$-fold CV}
