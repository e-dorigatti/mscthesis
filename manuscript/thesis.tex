...
Even though there is agreement on form of the universal functions in the Monin-Obukhov similarity theory, there is still debate on the exact values of the coefficients, with different experiments resulting in different values [citation]. In order to ensure a fair comparison with the results of this work, we fit the universal functions to the data from the Cesar database.
...
0 & 0 & - 2 c d \xi \tau^{d - 1} & \tau^{d} \log{\tau} \\
0 & 0 & 0 & 0 \\
- 2 c d \xi \tau^{d - 1} &
0 & \frac{2 a d \xi \tau^{d}}{c^{4} \xi^{2}+\tau} \left(2 c^{2} d \xi - c^{2} \xi - 1\right) & - 2 a c \xi \tau^{d - 1} \left(d \log{\tau} + 1\right) \\
\tau^{d} \log{\tau} & 0 &
- 2 a c \xi \tau^{d - 1} \left(d \log{\tau} + 1\right) &
\tau^{d} \log^{2}{\tau}
\end{bmatrix}
\end{equation}
