...
\input{include/repeated_cv_count.tex}
\end{table}


As argued in section \ref{sec:effect_size}, we normalize the effect size with the standard deviation of the treatment. In table \ref{tbl:baseline_comparison}, it is clear that the MOST estimator is the control group, and the other estimators are different treatments, however this distinction is not so clear-cut in tables \ref{tbl:trend_comparison} and \ref{tbl:fset_comparison}, therefore these tables report both effect sizes.  The goal of the tables is to answer one-sided questions of the form\\

\emph{Q: How much better is the treatment compared to the control?} \\

\noindent with an answer like: \\
 
\emph{The treatment is $\delta$ better than the control, and with high probability better than $\delta_{low}$}. \\

\noindent This answer can be transformed into the answer to: \\

\emph{$Q^\prime$: How much better is the control compared to the treatment?} \\

\noindent as follows: \\
 
\emph{The control is $-\delta$ better than the treatment, but with high probability no better than $-\delta_{low}$}.\\
 
\noindent Comparing two samples $A$ and $B$ using each of them both as control and treatment allows us to answer the following four questions:

\begin{enumerate}[leftmargin=*,align=LeftAlignWithIndent]
\item[$Q1$.] How much better is $A$ compared to $B$, when $A$ is the treatment?
\item[$Q1^\prime$.] How much better is $B$ compared to $A$, when $A$ is the treatment?
\item[$Q2$.] How much better is $B$ compared to $A$, when $B$ is the treatment?
\item[$Q2^\prime$.] How much better is $A$ compared to $B$, when $B$ is the treatment?
\end{enumerate}

Note that it is wrong to combine the answers to $Q$ and $Q^\prime$ to obtain a two-sided CI, for the simple reason that these answers use two different treatments. In other words, one should refrain from combining CIs, but choose the appropriate one \emph{after} determining which question is of interest \emph{and} which interpretation is appropriate.
...
trend would be better included using some sort of time series prediction together with the features we considered in this work
