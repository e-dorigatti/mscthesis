...
The differences are quantified using a linear contrast of means, and the 95\% confidence interval for the population effect size. We use the formula suggested by \cite{effect_size}, which do not assume that the samples originate from two populations with the same variance, which is hardly the case in our setting since we are comparing different estimators and/or different feature sets. Given two independent samples of $n_1$ and $n_2$ observations, with sample mean $\mu_1$ and $\mu_2$ and standard deviations $\hat{\sigma_1}$, $\hat{\sigma_2}$, the effect size is computed as follows:

\begin{equation}
\label{eq:effect_size}
\hat{\delta}=\frac{\mu_1-\mu_2}{\hat{\sigma}}
\end{equation}

where $\hat{\sigma}$ is the average standard deviation: $\hat{\sigma}=[(\hat{\sigma_1}^2+\hat{\sigma_2}^2)/2]^{1/2}$. The variance of \ref{eq:effect_size} is given by


\begin{equation}
\text{Var}[\hat{\delta}]=\frac{\hat{\delta}^2}{8\hat{\sigma}^4}\left(
\frac{\hat{\sigma_1}^4}{n_1-1} + \frac{\hat{\sigma_2}^4}{n_2-1}
\right)
+\frac{\hat{\sigma_1}^2}{\hat{\sigma}^2(n_1-1)}
+\frac{\hat{\sigma_2}^2}{\hat{\sigma}^2(n_2-1)}
\end{equation}

and an approximate $100(1-\alpha)\%$ confidence interval can be obtained with

\begin{equation}
\hat{\delta}\pm\bm\Phi^{-1}\left(1-\frac{\alpha}{2}\right)\cdot\sqrt{\text{Var}[\hat\delta]}
\end{equation}

where $\bm\Phi$ is the cumulative distribution function of the standardized Normal.
...
and \ref{tbl:gbr_results}, while figures \ref{fig:ridge_results_most}, \ref{fig:knn_results_most} and \ref{fig:gbr_results_most} show boxplots of these metrics for the MOST validity range, and figures \ref{fig:ridge_results_all}, \ref{fig:knn_results_all} and \ref{fig:gbr_results_all} do so for the whole dataset. Table \ref{tbl:baseline_comparison} compares each estimator and feature set combination with the MOST baseline for both the full dataset and the data in the MOST validity range, while table \ref{tbl:fset_comparison} compares the errors of successive feature sets, and finally table \ref{tbl:trend_comparison} shows the error difference when the the feature sets are augmented with the trend information.

\begin{table}
\caption{Effect sizes comparing the MSE scores of the MOST estimator with each combination of estimator, feature set, and dataset; each comparison used the trend only if it resulted in a lower error. A positive effect size indicates that the estimator is better than the MOST baseline.}
...
\begin{table}
\caption{Effect sizes comparing the MSE scores of each feature set with the previous set (i.e. F2 is compared with F1, F3 with F2, and so on). A positive effect size indicates that the feature set has a lower error than the previous one.}
...
\begin{table}
\caption{Effect sizes comparing the MSE scores of each feature set with and without trend. A positive effect size indicates that the trend reduces the error.}
