...
If we assume a Gaussian likelihood like the one in equation \ref{eq:likelihood}, and a spherical Normal prior distribution $p(\bm\theta)=\mathcal{N}(\bm 0, \lambda\bm I)$, the MAP estimation of equation \ref{eq:map} becomes

\begin{equation}
\label{eq:least_squares}
\bm\theta^*=\argmin_{\bm\theta}\ \sum_{n=1}^N\left( y(\bm x_n,\theta)-t_n\right)^2+\lambda\bm\theta^\intercal\bm\theta
\end{equation}

i.e. the minimization of the sum-of-squares error. The parameter $\beta$ can be estimated as well in a similar way, if necessary, and a fully Bayesian treatment allows to estimate $\lambda$, too. Depending on the model $y(\bm x_n,\bm\theta)$, \ref{eq:least_squares} can be solved analytically to yield a closed-form solution for $\theta$; this is the case for linear regression, where $y(\bm x_n,\bm\theta)=\bm\theta^\intercal\bm x_n$ and the optimal parameters are $\bm\theta^*=(\bm X^\intercal\bm X+\lambda\bm I_N)^{-1}\bm X^\intercal\bm t$.
...
Even though there is agreement on form of the universal functions in the Monin-Obukhov similarity theory, there is still debate on the exact values of the coefficients, with different experiments resulting in different values. In order to ensure a fair comparison with the results of this work, we use fit the universal functions to the data from the Cesar database.
...
Following the approach outlined in section \ref{sec:regression_fit}, the coefficients $a$, $b$, $c$ and $d$ can be found by minimizing the L2-regularized mean squared error using equation \ref{eq:least_squares_gradient}. The partial derivatives are:

\begin{align}
\frac{\partial}{\partial a}\phi(\xi)&=\begin{cases}
\mathrlap{1}\hphantom{a(1-c^2\xi)^d\ln(1-c^2\xi)} & \xi\geq 0 \\
(1-c^2\xi)^d &\xi<0
\end{cases} \\
\frac{\partial}{\partial b}\phi(\xi)&=\begin{cases}
\xi & \xi\geq 0 \\
\mathrlap{0}\hphantom{a(1-c^2\xi)^d\ln(1-c^2\xi)} &\xi<0
\end{cases}\\
\frac{\partial}{\partial c}\phi(\xi)&=\begin{cases}
\mathrlap{0}\hphantom{a(1-c^2\xi)^d\ln(1-c^2\xi)} & \xi\geq 0 \\
-2acd\xi(1-c^2\xi)^{d-1} & \xi<0
\end{cases} \\
\frac{\partial}{\partial d}\phi(\xi)&=\begin{cases}
0 & \xi\geq 0 \\
a(1-c^2\xi)^d\ln(1-c^2\xi) & \xi<0
\end{cases}
\end{align}
