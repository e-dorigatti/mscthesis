...
The goal of supervised learning is to find the best approximation for an unknown mapping $f*\rightarrow\mathcal{X}\times\mathcal{T}$ among a family of available functions $\mathcal{F}$, using a set of $N$ training examples generated from an unknown joint probability distribution $p(\mathcal{X},\mathcal{T})$. The quality of a guess $y_n$ for the true value $t_n$ is computed using a loss function $l:\mathcal{T}\times\mathcal{T}\rightarrow\mathcal{R}$
