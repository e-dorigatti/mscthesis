...
\begin{table}[p]
\caption{Effect sizes comparing the MSE scores of each feature set with and without trend; a positive effect size indicates that the trend (treatment) reduces the error compared to not using the trend (control). The first number is the lower bound for the right 95\% CI interval, and the second number is the estimated population effect size; in bold are the CIs that do not contain 0. Using the trend does not improve the performance.}
\label{tbl:trend_comparison}
\input{include/trend_comparison}
\end{table}

\begin{table}[p]
\caption{Effect sizes comparing the MSE scores of all pairs of feature sets; in light of table \ref{tbl:trend_comparison}, the trend is not included in the features. The first number is the lower bound for the right 95\% CI interval, and the second number is the estimated population effect size; positive effect size denotes improvement, in bold are the CIs that do not contain 0. The only feature sets that do not bring an improvement are F2 over F1 and F4 over F3.}
...
Refer to appendix \ref{apx:all_results} for detailed results, including all metrics for all combinations of estimator, dataset, trend and features.
