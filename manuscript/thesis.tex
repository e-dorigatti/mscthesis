...
Random forests \citep{random_forests} are an ensemble approach to prediction whose output values are the combination of the output of multiple decision trees. In general, ensemble methods improve the performance of a single model by combining the output of many models trained on random subsets of features and/or samples.
...
The parameter $\sigma_2$ controls the noise, and depends on the instrument used to measure the modeled quantity; for example, for wind, we have $\sigma_2^2=\SI{0.1}{\square\meter\per\square\second}$; the other parameters are found by optimizing the marginal likelihood.

Figure \ref{fig:kernel} shows the prior distribution of a Gaussian process with this kernel, as well as the value of the kernel for some choices of $z_1$ and $z_2$.
...
All models but neural networks are fit using nested cross validation with random hyper-parameter search. This procedure detailed in section \ref{sec:perf_eval}, but we list here the distributions of the hyper-parameters that we used in the random search:

\begin{itemize}
\item Ridge regression and Monin-Obukhov estimator: the only hyper-parameter to tune is the regularization coefficient, for which we use a log$_{10}$-uniform distribution from $10^{-6}$ to $10_{1}$\footnote{a random variable $X$ has a log$_\beta$-uniform distribution from $a$ to $b$ if $\log_\beta X$ is uniformly distributed between $a$ and $b$. Equivalently, if $Y$ is uniform between $a$ and $b$, then $\beta^Y$ is log$_\beta$-uniform.}.
...
\item Gradient boosted trees: the number of estimators is a log$_10$-uniform from 10 to 1000, the number of features is uniform from 1 to all, the maximum depth is uniform from 2 to 10, the sub-sampling factor is uniform from 10\% to 100\%, and the loss function is either least squares or least absolute deviation. The learning rate was not tweaked, as there is a depe
\end{itemize}

\section{Performance Evaluation}
\label{sec:perf_eval}
The goal of performance evaluation is to obtain a realistic and unbiased estimate of the performance of a model on unseen data; we are not interested in the values of the hyper-parameters that yield this performance. For this reason, we perform nested cross validation, with the inner loop used to optimize the hyper-parameters through random search \citep{random_search}, and the outer loop used to evaluate the model. We use 10 folds in both the inner and outer CV, and test 60 different hyper-parameter combinations. This number was chosen based following the advice by \cite{random_search}, based on a simple probabilistic argument: if every combination has a 5\% probability of giving a model whose performance is within 5\% of the best attainable performance, then the probability that at least one in 60 trials finds a combination within 5\% of the optimal is $1-0.95^{60}\approx 0.95$.
Algorithm \ref{alg:nested_cv} shows in detail how nested cross validation with random search works.
...
Where $f_n$ is the predicted value for the test sample $\bm x_n$ with true value $t_n$, the squared error is $(f_n-t_n)^2$, the absolute error is $\vert f_n-t_n\vert$, the absolute percent error is $\vert 1-f_n/t_n\vert$
...
\For{$k_o\gets 1\ldots K_O$}\Comment{Outer $K_O$-fold cross validation}
	...
	\For{$r\gets 1\ldots R$}\Comment{Find best hyper-parameters on $D^o_T$}
		\State{$params\gets$ a random hyper-parameter combination}
		\State{$params\_mse\gets 0$}
		\For{$k_i\gets 1\ldots K_I$}\Comment{Evaluate $params$ with $K_I$-fold CV}
			\State{Generate $k$-th inner fold $(D^i_T,D^i_V)$ from $D^o_T$}
		...
		\If{$params\_mse<best\_mse$}\Comment{MSE comparison on $K$-fold CV result}
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{Friction velocity versus temperature (left) and wind speed (right). There is a clear subgroup where the wind and $u^*$ are almost perfectly correlated; this is because of the gap-filling technique discussed in section \ref{sec:gap_filling}. Additionally, there are three subgroups with different slopes of the regression line; they differ widely in fluxes (both surface and turbulent) and radiation.}
...
\begin{table}[]
\centering
\caption{Evaluation metrics for the Ridge linear regression estimator on F5 with trend}
\label{tbl:ridge_results_f5t}
\begin{tabular}{r|l|l}
 & Mean & Std.   \\ \hline
...
\begin{table}[]
\centering
\caption{Evaluation metrics for the k-nearest neighbors estimator on F5 with trend}
...
\begin{table}[]
\centering
\caption{Evaluation metrics for the gradient boosting estimator on F5 with trend}
