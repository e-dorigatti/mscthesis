...
Python itself, operating systems, distributed computing platforms, and so on. Among them, I am extremely grateful to team behind the Hops platform\footnote{\url{http://www.hops.io/}}, for giving me access to an astonishing amount of computational resources and their support in using them.
...
The author strongly aligns with the Open Science movement, according to which scientific research should be made as open and accessible as possible, so that it can be examined, extended and improved by as many people as possible. This is what allowed mankind to become what it is today, and it should not change. For this reason, this work was performed entirely with Free and Open Source tools, providing a low barrier to entry, and focusing on reproducibility of the results. The entirety of the analysis, from the very data to the generation of plots and tables, is openly accessible\footnote{\url{https://github.com/e-dorigatti/mscthesis}} and can be performed by anyone with enough computational resources.

[todo is this paragraph necessary?] As noted in the introduction, climate scientists have found evidence of a recent change in our planet's climate, which is predicted to considerably impact life on Earth in the coming centuries [check and cite]: understanding climate is very important, for it could suggest ways of preserving our only home.
...
ethics: fair share of cluster resources? XD
...
The differences are quantified using the effect size, as detailed in section \ref{sec:effect_size}; since we are looking for an improvement, we compute right CI for the population effect size, i.e. we only compute a (probabilistic) lower bound.
...
\begin{table}
\caption{Effect sizes and their 95\% right CI comparing the MSE scores of all pairs of feature sets, for the GB estimator and for the MOST dataset. The control is on rows and the treatment is on columns, and the effect size is positive when the treatment has a smaller MSE than the control; in bold are the CIs that do not contain 0.}
...
\begin{table}
\caption{Descriptive statistics of the best MSE achieved by each estimator among all feature sets with and without trend, and effect size comparing it with the MOST baseline; positive effect size denotes improvement, in bold are the CIs that do not contain 0.}
...
\begin{table}
\caption{Effect sizes and their 95\% right CI comparing the MSE scores of each feature set with and without trend. A positive effect size indicates that the trend reduces the error; in bold are the CIs that do not contain 0.}
