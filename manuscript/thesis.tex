...
\section*{Summary}
The study of climate heavily relies on simulations. In order to successfully simulate the evolution of the global climate over long periods of time, the simulations have to use a relatively coarse grid, in which points are horizontally separated by several kilometers. Many important phenomena of the atmosphere, though, happen at much smaller scales, and the only way to incorporate them in the simulation is to rely on parametrizations, i.e. describing their effect based on macro parameters.

The turbulence resulting from the interaction between the atmosphere and the surface is an example of such phenomena that happens at spatial and temporal scales too small to be simulated. One of the quantities of interest arising from turbulence is the momentum flux-profile relationship, which relates the transport of momentum (flux) and the change of wind speed with altitude (profile). This quantity can be computed following the Monin-Obukhov Similarity Theory, according to which it is a function of the altitude divided by a characteristic length scale derived from other turbulence parameters. However, this theory is only applicable in a restricted range of conditions, and produces predictions that are accurate only up to 10-20\%.
...
Where $\mu_t$ and $\mu_c$ are the means of the treatment and control groups respectively, and $\sigma$ is a standard error that can be computed in a great deal of ways. Generally, the effect size is computed to be positive when the treatment group represent an improvement over the control group.

Following the American Psychological Association’s Task Force on Statistical Inference [citation], there is a departure from the standard procedure of reporting p-values, test statistics and critical values resulting from hypothesis testing towards reporting effect sizes and their confidence intervals, in addition or in alternative to the usual statistics. However, effect sizes can be as misleading as significance testing when the assumptions underlying its expression are violated; commonly used effect size measures, such as Cohen's $d$ [citation], assume both populations are normally distributed and have the same standard deviation\footnote{a property called \emph{homoscedasticity} (contrary: heteroscedasticity)}. Alternatives that do not assume equal standard deviation exist [citation 1,2,3], but they all require normality, and generally use approximate distributions of the standard error and/or the confidence interval (or both).

In order not to assume normality, we compute the confidence interval for the effect size using the bootstrap percentile method. This method estimates the population value of any statistic $\theta$ and its confidence interval using a single sample of $N$ observations, by extracting $M$ new samples (with replacement) of size $N$ from it and computing $\theta$ on each resample. The population statistic is estimated by the average of the $m$ sample statistics, and the extremes of the (central) $100(1-\alpha)\%$ confidence interval are respectively the $100(\alpha/2)$ and $100(1-\alpha/2)$ percentiles of those values. Usually $M$ is in the order of $10^2$ or $10^3$, enough to guarantee a certain degree of stability in these estimates.

The sample effect size of equation \ref{eq:base_es} is not robust to outliers when it is computed with the sample means and standard deviations. Since we do not assume normality outliers are expected, therefore we use measures of location that are not heavily affected by them, namely the sample trimmed mean and the sample Winsorized standard error. The $100\gamma \%$ trimmed mean $\overline{X}_\gamma$ of a sample is simply the mean of that sample after discarding the $100\gamma\%$ largest and the $100\gamma\%$ smallest observations\footnote{this is common practice, for example, when scoring in the modern Olympiads, where the smallest and largest scores are not considered.}. The standard error of the sample trimmed mean is obtained from the Winsorized sample standard deviation; let $X_{(\gamma)}$ and $X_{(1-\gamma)}$ be the $100\gamma$ and $100(1-\gamma)$ percentiles, and define the Winsorized observations as follows:
...
=(1-2\gamma)\frac{\overline{T}_\gamma-\overline{C}_\gamma}{\hat{\sigma}_W^2}
\end{equation}

where $T_1,\ldots,T_N$ and $C_1,\ldots,C_M$ are the \emph{independent} samples of treatment and control groups respectively. See \cite{modern_stat} for an introduction to robust statistical methods, including the ones just described.
...
The differences are quantified using the effect size, as detailed in section \ref{sec:effect_size}.

\chapter{Results}
\label{ch:results}
todo
also mention python libraries used
...
\textbf{Air Press.} & 1,009.6 & 69.0 & 0.0 & 1,008.3 & 1,014.8 & 1,020.7 & 1,046.4 \\
...
\textbf{Air Dens.} & 0.3 & 0.1 & 0.0 & 0.2 & 0.3 & 0.3 & 2.8 \\
...
\subcaptionbox{Wind\label{fig:tsne_wind}}{\includegraphics[width=0.25\textwidth]{images/tsne_wind}}\\
...
\section{Predicting the Flux-Profile Relationship}
In this section, we present and compare the evaluation metrics obtained by the estimators on the different configurations of features and data. We will refer to the interval $-2\leq\xi\leq1$ as the \emph{MOST validity range}, abbreviated as \textbf{MOST} in the tables; the proportion of data falling in this interval is 83.6\%. Unless specified otherwise, the metrics are presented as the mean over the ten outer validation folds followed by the standard deviation, in parenthesis, whereas the effect sizes are presented followed by the 95\% confidence interval in parenthesis.


\subsection{MOST Estimator Baseline}
...
\begin{table}[]
\centering
\caption{Evaluation metrics for the MOST estimator on both datasets.}
...
The averaged evaluation metrics for the Ridge, kNN and GB estimators are presented in tables \ref{tbl:ridge_results}, \ref{tbl:knn_results}
and \ref{tbl:gbr_results}, while figures \ref{fig:ridge_results_most}, \ref{fig:knn_results_most} and \ref{fig:gbr_results_most} show boxplots of these metrics for the MOST validity range, and figures \ref{fig:ridge_results_all}, \ref{fig:knn_results_all} and \ref{fig:gbr_results_all} do so for the whole dataset. Table \ref{tbl:baseline_comparison} compares each estimator with the MOST baseline for both the full dataset and the data in the MOST validity range, while table \ref{tbl:fset_comparison} compares the errors of successive feature sets, and finally table \ref{tbl:trend_comparison} shows the error difference when the the feature sets are augmented with the trend information.

\begin{table}
\caption{Descriptive statistics of the best MSE achieved by each estimator among all feature sets with and without trend, and effect size comparing it with the MOST baseline. A positive effect size denotes improvement, and its 95\% confidence interval is in parenthesis.}
...
\begin{table}
\caption{Effect sizes and their 95\% confidence interval comparing the MSE scores of all pairs of feature sets, for each estimator and for the MOST dataset. The control is on rows and the treatment is on columns, and the effect size is positive when the treatment has a smaller MSE than the control; in bold are the combinations whose confidence interval does not contain 0.}
\label{tbl:fset_comparison}
\input{include/fset_comparison_most}
\end{table}

\begin{table}
\caption{Effect sizes and their 95\% confidence interval comparing the MSE scores of all pairs of feature sets, for each estimator and for the full dataset. The control is on rows and the treatment is on columns, and the effect size is positive when the treatment has a smaller MSE than the control; in bold are the combinations whose confidence interval does not contain 0.}
\label{tbl:fset_comparison}
\input{include/fset_comparison_nomost}
\end{table}


\begin{table}
\caption{Effect sizes and their 95\% confidence interval comparing the MSE scores of each feature set with and without trend. A positive effect size indicates that the trend reduces the error; in bold are the combinations whose confidence interval does not contain 0.}
...
\begin{table}[h]
\caption{Evaluation metrics for the k-Nearest Neighbors Regressor estimator.}
...
\begin{figure}[h]
\subcaptionbox{Mean Squared Error\label{fig:result_GB_most_mean_squared_error}}{\includegraphics[width=0.49\textwidth]{images/result_GB_most_mean_squared_error}}\hfill
\subcaptionbox{$R^2$ Score \label{fig:result_GB_most_r2_score}}{\includegraphics[width=0.49\textwidth]{images/result_GB_most_r2_score}}\\

\subcaptionbox{Mean Absolute Error\label{fig:result_GB_most_mean_absolute_error}}{\includegraphics[width=0.49\textwidth]{images/result_GB_most_mean_absolute_error}}
\subcaptionbox{Median Absolute Error\label{fig:result_GB_most_median_absolute_error}}{\includegraphics[width=0.49\textwidth]{images/result_GB_most_median_absolute_error}}\\

\subcaptionbox{Mean Absolute Percent Error\label{fig:result_GB_most_mean_abs_percent_error}}{\includegraphics[width=0.49\textwidth]{images/result_GB_most_mean_abs_percent_error}}\hfill
\subcaptionbox{Median Absolute Percent Error\label{fig:result_GB_most_median_abs_percent_error}}{\includegraphics[width=0.49\textwidth]{images/result_GB_most_median_abs_percent_error}}\\
...
\begin{figure}[h]
\subcaptionbox{Mean Squared Error\label{fig:result_GB_nomost_mean_squared_error}}{\includegraphics[width=0.49\textwidth]{images/result_GB_nomost_mean_squared_error}}\hfill
\subcaptionbox{$R^2$ Score \label{fig:result_GB_nomost_r2_score}}{\includegraphics[width=0.49\textwidth]{images/result_GB_nomost_r2_score}}\\

\subcaptionbox{Mean Absolute Error\label{fig:result_GB_nomost_mean_absolute_error}}{\includegraphics[width=0.49\textwidth]{images/result_GB_nomost_mean_absolute_error}}
\subcaptionbox{Median Absolute Error\label{fig:result_GB_nomost_median_absolute_error}}{\includegraphics[width=0.49\textwidth]{images/result_GB_nomost_median_absolute_error}}\\

\subcaptionbox{Mean Absolute Percent Error\label{fig:result_GB_nomost_mean_abs_percent_error}}{\includegraphics[width=0.49\textwidth]{images/result_GB_nomost_mean_abs_percent_error}}\hfill
\subcaptionbox{Median Absolute Percent Error\label{fig:result_GB_nomost_median_abs_percent_error}}{\includegraphics[width=0.49\textwidth]{images/result_GB_nomost_median_abs_percent_error}}\\
