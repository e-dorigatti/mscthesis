...
The differences are quantified using the effect size as described in section \ref{sec:effect_size}; since we are looking for an improvement, we compute the right 95\% confidence interval for the population effect size. In other words, besides providing a point estimate for the population effect size $\delta_{pop}$, we also provide an interval $[\delta_{low}, \infty)$ that contains it 95\% of the times. We say the treatment is better if $\delta_{low}>0$.
...
\toprule
& z & LG & GP & z & LG & GP & z & LG & GP \\\midrule
...
We refer to the interval $-2\leq\xi\leq1$ as the \emph{MOST validity range}, or \emph{MOST dataset} in the tables; it contains 1,297,841 samples, 83.6\% of the total. The performance of the MOST estimator is shown in table \ref{tbl:most_results}; we see a considerable increase in the MSE when the full dataset is used; this is not surprising, considering that, with $\xi>1$, $\phi_m$ tends to level off, and there are a considerable number of outliers in the range $-20\leq\xi\leq-2$ (see figure \ref{fig:most_full}) that cannot be fitted by the MOST estimator. Figure \ref{fig:most_scatter} shows $\phi_m$ as a function of $\xi$ as computed in our dataset, as well as the predictions of equation \ref{eq:phi_m_coeff} and the fitted MOST estimator.
...
Table \ref{tbl:baseline_comparison} shows descriptive statistics of the best MSE obtained by each estimator for each datasets, among all the feature sets with and without trend, as well as the effect size comparing it with the MOST baseline. 

todo procedure for choosing best MSE
