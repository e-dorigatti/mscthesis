...
Where $\mathbbm{1}[z_1=z_2]$ is an indicator function whose value is 1 if $z_1=z_2$, and 0 otherwise. The parameter $\sigma_2$ controls the noise, and depends on the instrument used to measure the modeled quantity; for example, for wind, we have $\sigma_2^2=\SI{0.1}{\square\meter\per\square\second}$; the other parameters are found by optimizing the marginal likelihood. Figure \ref{fig:kernel} shows the prior distribution of a Gaussian process with this kernel, as well as the value of the kernel for some choices of $z_1$ and $z_2$.
...
\label{sec:perf_eval}
The goal of performance evaluation is to obtain a realistic and unbiased estimate of the performance of a model on unseen data; we are not interested in the values of the hyper-parameters that yield this performance. For this reason, we perform nested cross validation, with the inner loop used to optimize the hyper-parameters through random search \citep{random_search}, and the outer loop used to evaluate the model. We use 10 folds in both the inner and outer CV, and test 60 different hyper-parameter combinations for all estimators but gradient boosted trees, for which we use 180. This number is based on a simple probabilistic argument\footnote{\url{https://stats.stackexchange.com/a/209409/141343}}: if a combination has a 5\% probability of giving a model whose performance is within 5\% of the best attainable performance, then the probability that at least one in 60 trials finds a combination within 5\% of the optimal is $1-0.95^{60}\approx 0.95$. Clearly, these numbers are arbitrary, but even a probability of 1\% would give a success probability of 45\%. Algorithm \ref{alg:nested_cv} shows in detail how nested cross validation with random search works.
...
Talking about speed of wind, figure \ref{fig:ustar_wind} shows some measurements exceeding \SI{25}{\meter\per\second}; according to the Beaufort scale, holding umbrellas and walking become hard with wind at \SI{15}{\meter\per\second}, but those measurements are not issues with the data. Figure \ref{fig:cyclone_jeanett} shows the highest wind speed present in the Cesar database to date, corresponding to cyclone Jeanett.
