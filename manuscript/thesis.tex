...
The models that we use are ridge regression, k-nearest neighbors, random forests, and neural networks. Due to the size of the dataset, we cannot use kernel-based algorithms such as SVM and Gaussian Processes, as they require $\Theta(N^2)$ memory to store the $N\times N$ kernel matrix, and $O(N^3)$ time to invert it. Random forests were preferred over Gradient boosted trees because they can be constructed much more efficiently, since all the trees can be constructed in parallel. Gradient boosted trees, on the other hand, have to be constructed incrementally
