...
fit metrics on training set
     - average r squared for positive and negative z/L are -0.6 and -0.8 with std 1.5 and 4 respectively
     - % error on the wind speed is between 10 and 80
    basically bad fit most of the times, highly depends on conditions
on the other hand
    this allows us to get gradient at 10 meters without worrying about roughness length
        distribution of phi_m at 10 meters is similar to distribution at 20 and 40 meters
    compare the CDF of phi_m with this method and with finite differences (>10 meters)
        very veery similar!
    they are correlated to some extend (the r-squared is 0.59)
        absolute difference is high <=> the r-squared of the modeled wind speed is high
        absolute difference is low <=> the r-squared is all over the place
            meaning that when the profile is not logarithmic, both methods suck
        there seem to be three clusters in the correlation graph
            model >> fdiff, model ~ fdiff, model << fdiff
            tried to predict difference (<-1, >1, -1<<1), with decision tree, but had no success
...
NB a mse of 0.75 corresponds to 0.22/0.23 normalized mse
