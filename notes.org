...
L is a length scale that depends on the local conditions
    ...
    when z << L, shear dominates
        we have the traditional log profile of wind
        conditions are stable, L positive
    when z >> L, buoyancy dominates
        main source of energy is convection
        unstable conditions, L negative
    actually, z/L is the production of turbulent kinetic energy due to buoyancy (hogstrom 1996, sec. 5.1)
